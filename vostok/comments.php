...
<?php if ($comments) : ?>
	<div id="comments">
		<h3><?php comments_number(__('','vostok'), __('1 comment:','vostok'), __('% comments:','vostok') );?></h3>
		<ol>
		<?php foreach ($comments as $comment) : ?>
			<li id="comment-<?php comment_ID() ?>"  class="comment <?php if (get_comment_author_email() == get_the_author_meta('user_email')) { echo ('highlighted'); }?>">
				...
				<?php endif; ?>
				<cite><span class="date"><?php comment_date(__('l, F j, Y','vostok')) ?></span><?php comment_author_link() ?> says:</cite>
				<blockquote>
					<?php comment_text(); ?>
				</blockquote>
			</li>
